...
-- -----------------------------------------------------
-- Table `Project_DB`.`illnesss`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Project_DB`.`illnesss` ;

CREATE TABLE IF NOT EXISTS `Project_DB`.`illnesss` (
  `illneess_code` INT NOT NULL AUTO_INCREMENT,
  `illness_desc` VARCHAR(45) NULL,
  `illness_name` VARCHAR(45) NULL,
  PRIMARY KEY (`illneess_code`),
  UNIQUE INDEX `Illneess_Code_UNIQUE` (`illneess_code` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Project_DB`.`medication_reactions` (
  ...
  `fk_reacting_medication` INT NULL,
  `severity` VARCHAR(45) NULL,
  PRIMARY KEY (`fk_initial_medication_code`),
...
CREATE TABLE IF NOT EXISTS `Project_DB`.`nurse_inpatient_assignments` (
  ...
  INDEX `Patient_ID_idx` (`fk_assignment_patient_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `Project_DB`.`owners` (
  ...
  UNIQUE INDEX `Name_UNIQUE` (`fk_owner_name` ASC) VISIBLE,
  CONSTRAINT `fk_phys_Name`
    FOREIGN KEY (`fk_owner_name`)
    REFERENCES `Project_DB`.`physician_owners` (`fk_own_employee_name`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Name_Corp`
    FOREIGN KEY (`fk_owner_name`)
    REFERENCES `Project_DB`.`medical_corporations` (`corporation_name`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Project_DB`.`patient_illness` (
  ...
  `fk_illnesses_consultation_number` INT NOT NULL,
  PRIMARY KEY (`fk_illnesses_patient_id`, `fk_illnesses_illness_code`, `fk_illnesses_consultation_number`),
  ...
  CONSTRAINT `fk_ill_Illness_Code`
    ...
    REFERENCES `Project_DB`.`illnesss` (`illneess_code`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Project_DB`.`recorded_surgeries` (
  ...
  `date` DATETIME NULL,
  PRIMARY KEY (`fk_recorded_suregon_id`, `fk_recorded_surgery_code`, `fk_recorded_patient_id`),
...
CREATE TABLE IF NOT EXISTS `Project_DB`.`salaries` (
  ...
  `salary` INT NULL,
  `fk_salary_employee_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `Project_DB`.`surgery_schedule` (
  `fk_schedule_operating_theater` INT NOT NULL,
  `date` DATE NULL,
  ...
  PRIMARY KEY (`fk_schedule_operating_theater`, `fk_schedule_surgeon_id`, `fk_nurse_id_2`, `fk_nurse_id_1`, `fk_schedule_surgery_code`),
...
CREATE TABLE IF NOT EXISTS `Project_DB`.`surgery_types` (
  ...
  `anatomical_location` VARCHAR(45) NOT NULL,
  `special_needs` VARCHAR(45) NOT NULL,
  `category` VARCHAR(45) NOT NULL,
  `type_name` VARCHAR(45) NULL,
  PRIMARY KEY (`surgery_code`, `category`),
...
USE `Project_DB`;
CREATE  OR REPLACE VIEW heart_risk_view (patient_name, hdl, ldl, triglycerides, total_cholesterol, heart_risk) AS 
SELECT patient_personal_data.patient_name, hdl, ldl,triglycerides, (hdl+ldl+(.2*triglycerides)), 
	(
    CASE
		WHEN patient_medical_data.high_risk = 'TRUE' THEN 'High'
	ELSE
    	(CASE
	...
	END)
FROM cholesterol, patient_personal_data, patient_medical_data
WHERE cholesterol.fk_cholesterol_patient_id = patient_personal_data.patient_id AND patient_personal_data.patient_id = patient_medical_data.fk_medical_data_patient_id;
